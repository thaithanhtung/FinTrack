...
export function ComparisonChart({ date1, date2, type, goldType, brand }: ComparisonChartProps) {
  ...
  const worldHistoryQuery = useQuery({
    queryKey: ['comparisonWorldHistory', startDate.toISOString(), endDate.toISOString()],
    queryFn: () => getWorldGoldHistory(startDate, endDate),
    enabled: type === 'world',
  })

  const vnHistoryQuery = useQuery({
    queryKey: ['comparisonVnHistory', startDate.toISOString(), endDate.toISOString(), goldType, brand],
    queryFn: () => getVNGoldHistory(startDate, endDate, goldType, brand),
    enabled: type === 'vn',
  })

  const historyData = type === 'world' ? worldHistoryQuery.data : vnHistoryQuery.data
  const isLoading = type === 'world' ? worldHistoryQuery.isLoading : vnHistoryQuery.isLoading
