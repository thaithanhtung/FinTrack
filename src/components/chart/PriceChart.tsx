...
function CustomTooltip({ active, payload }: CustomTooltipProps) {
  ...
  return (
    <div className='bg-white shadow-lg rounded-lg p-3 border border-gray-200'>
      ...
      <p className='font-bold text-gray-900'>{formatUSD(data.value)}</p>
    </div>
  );
}
  ...
  if (isLoadingAvailability) {
    return (
      <Card>
        ...
        <Loading size='md' text='Đang kiểm tra dữ liệu...' />
      </Card>
    );
  }
    ...
    <Card>
      ...
      {chartData.length >= 2 && (
        <div className='mt-4 pt-4 border-t border-gray-100 grid grid-cols-3 gap-2 text-center'>
          <div>
            <p className='text-xs text-gray-500'>Mở cửa</p>
          ...
          <div>
            ...
            <p className={`font-semibold ${isUp ? 'text-up' : 'text-down'}`}>
              {formatUSD(lastPrice)}
            </p>
          </div>
          <div>
